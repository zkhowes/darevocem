...
Google Sign-In screen. Clean, large buttons, the user-friendly.
...
const SYSTEM_PROMPT = `You predict the next word or phrase in a sentence being composed by the user, who has aphasia. She selects an intent, then you predict what comes next for each slot.
...
- Weight the user's personal patterns heavily — her history matters more than general language probability.
...
the user's preferences only. Large controls, clear labels. Reference: spec section 12.
